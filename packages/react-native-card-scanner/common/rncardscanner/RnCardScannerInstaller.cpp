        ...
        [](jsi::Runtime &runtime, const jsi::Value &thisValue,
          ...
          try {
            ...
            for (size_t i = 0; i < inferenceResult.embedding.size(); i++) {
              embedding.setValueAtIndex(runtime, i,
                                        jsi::Value(static_cast<double>(
                                            inferenceResult.embedding[i])));
            }
        ...
        *jsiRuntime,
        ...
        [](jsi::Runtime &runtime, const jsi::Value &thisValue,
          ...
          if (count != 2 || !args[0].isString() || !args[1].isString()) {
            throw jsi::JSError(runtime,
                               "runInferenceOnImage expects (modelPath: "
          ...
          try {
            auto inferenceResult =
                cardscanner::CardScanner::runInferenceOnImage(modelPath,
                                                              imagePath);
          ...
          try {
            ...
            for (size_t i = 0; i < segResult.detections.size(); i++) {
              if (!segResult.detections[i].dewarpedCard.empty()) {
                ...
                bool saved = cv::imwrite(dewarpPath,
                                         segResult.detections[i].dewarpedCard);
                ...
                dewarpedPaths.setValueAtIndex(
                    runtime, i,
                    jsi::String::createFromUtf8(runtime, dewarpUri));
              } else {
                std::cout << "Card " << i
        ...
        try {
          auto pipelineResult = cardscanner::CardRecognitionPipeline::recognize(
            ...
            timingBreakdown.setProperty(
                ...
                jsi::Value(
                    pipelineResult.timingBreakdown.yoloPostprocessingMs));
            timingBreakdown.setProperty(
                runtime, "yoloTotalMs",
            ...
            timingBreakdown.setProperty(
                ...
                jsi::Value(
                    pipelineResult.timingBreakdown.embeddingInferenceMs));
            timingBreakdown.setProperty(
            ...
            throw jsi::JSError(
                runtime, std::string("Card recognition failed: ") + e.what());
          }
        });
