...
dto::FABColorInfo FABColorProcessor::processColorVariant(
  ...
  if (cardMatches.size() >= 2) {
    ...
    if (scoreDifference > disambiguationThreshold) {
      return dto::FABColorInfo(); // Skip color detection
    }
  ...
  try {
    log(LOG_LEVEL::Debug,
        "[RNCardScanner] Detecting FAB color variant with disambiguation "
        "threshold: %.2f",
