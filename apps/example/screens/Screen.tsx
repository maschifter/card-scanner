...
import { View, Text, Button, StyleSheet, Platform, ScrollView, Image } from 'react-native';
import {
  runInference,
  runInferenceOnImage,
  InferenceResult,
...
import { cacheDirectory, copyAsync, documentDirectory, readAsStringAsync, writeAsStringAsync, getInfoAsync } from 'expo-file-system/legacy';
...
export default function MainScreen() {
  ...
  const [loadResult, setLoadResult] = useState<LoadEmbeddingsResult | null>(null);
  ...
  const handlePickImage = async () => {
    ...
    try {
      ...
      const [asset] = await Asset.loadAsync(require('../assets/embedding_model.pte'));
      ...
      console.log('Embedding extracted:', inferenceResult.embedding.length, 'dimensions');
  ...
  return (
    <ScrollView style={styles.scrollView} contentContainerStyle={styles.container}>
      ...
      <Button title="🚀 Recognize Cards (Full Pipeline)" onPress={handleRecognizeCards} color="#9C27B0" />
      <View style={styles.separator} />

      <Button title="🎯 Test YOLO Segmentation" onPress={handleRunYolo} color="#4CAF50" />
      <View style={styles.separator} />

      <Button title="📷 Pick Image from Gallery" onPress={handlePickImage} color="#FF6B6B" />
      <View style={styles.separator} />
      ...
      <Button title="📁 Load Card Embeddings (Select JSON)" onPress={handleLoadEmbeddings} color="#FF9800" />
      <View style={styles.separator} />
      <Button
      ...
      {loadResult && (
        <View style={styles.resultBox}>
          <Text style={styles.result}>
            Loaded {loadResult.loaded} cards
          </Text>
          <Text style={styles.info}>
      ...
      {yoloResult && (
        <View style={styles.resultBox}>
          ...
          <Text style={styles.timing}>
            Found {yoloResult.detections.length} card{yoloResult.detections.length !== 1 ? 's' : ''}
          </Text>

          {yoloResult.visualizedImagePath && (
            <View style={styles.imageContainer}>
              <Text style={styles.sectionTitle}>Detected Cards with Overlays:</Text>
              <Image
          ...
          <View style={styles.cardGrid}>
            {yoloResult.dewarpedCardPaths && yoloResult.dewarpedCardPaths.map((cardPath, index) => (
              <View key={index} style={styles.detectedCard}>
                <Text style={styles.detectedCardTitle}>
                  Card #{index + 1} ({(yoloResult.detections[index].box.conf * 100).toFixed(1)}%)
                </Text>
                {cardPath && (
                  <Image
                ...
                {!cardPath && (
                  <Text style={styles.info}>⚠️ Could not extract quadrilateral</Text>
                )}
              </View>
            ))}
          </View>
        ...
        <View style={styles.resultBox}>
          ...
          <View style={styles.timingBox}>
            ...
            <View style={styles.timingRow}>
              ...
              <Text style={styles.timingValue}>{recognitionResult.yoloTimeMs.toFixed(1)}ms</Text>
            </View>
            {recognitionResult.cards.map((card, idx) => (
              <View key={idx} style={styles.timingRow}>
                <Text style={styles.timingLabel}>  Card {idx + 1} Embedding:</Text>
                <Text style={styles.timingValue}>{card.embeddingTimeMs.toFixed(1)}ms</Text>
              </View>
            ))}
            <View style={[styles.timingRow, styles.timingTotal]}>
              <Text style={styles.timingLabelTotal}>Total Pipeline:</Text>
              <Text style={styles.timingValueTotal}>{recognitionResult.totalTimeMs.toFixed(1)}ms</Text>
            </View>
          </View>

          <Text style={styles.timing}>
            Found {recognitionResult.cards.length} card{recognitionResult.cards.length !== 1 ? 's' : ''}
          </Text>

          {recognitionResult.cards.map((card, cardIndex) => (
            <View key={cardIndex} style={styles.recognizedCard}>
              <Text style={styles.detectedCardTitle}>
                Card #{cardIndex + 1} (Confidence: {(card.conf * 100).toFixed(1)}%)
              </Text>
              <Text style={styles.info}>
